// Event Edit Screen
// User edits an existing event: location, time (24h dropdown),
// date (calendar modal), photo, title, description
// Uploads new photos to Cloudinary
...
import { getEvent, updateEvent } from '../../../services/everyoneService'
...
const EventEditScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      if (result.success) {
        ...
        setLink(e.link || '')
        setLinkLabel(e.linkLabel || '')
        // Convert legacy 24h time to AM/PM if needed
        let loadedTime = e.time || ''
        if (loadedTime && !loadedTime.includes('AM') && !loadedTime.includes('PM')) {
          const [h, m] = loadedTime.split(':').map(Number)
          const ampm = h >= 12 ? 'PM' : 'AM'
          const hour12 = h % 12 || 12
          loadedTime = `${hour12}:${m.toString().padStart(2, '0')} ${ampm}`
        }
        setTime(loadedTime)
        ...
        if (e.date) {
          const d =
            e.date instanceof Date ? e.date : e.date.toDate ? e.date.toDate() : new Date(e.date)
        ...
        if (e.imageUrl) {
          if (e.imageUrl.startsWith('placeholder:')) {
            setSelectedPlaceholder(e.imageUrl.replace('placeholder:', ''))
          } else {
  ...
  const handleSave = async () => {
    ...
    const result = await updateEvent(eventId, {
      ...
      zipCode,
      imageUrl,
      city: editIsGlobal ? 'Global' : editCity.trim(),
    })
          ...
          <View style={styles.mainContainer}>
            ...
            <View style={styles.cardContainer}>
              ...
              <TouchableOpacity style={styles.cardImageCell} onPress={handlePickImage}>
                {imageUri ? (
                  ...
                  <Image source={{ uri: existingImageUrl }} style={styles.cardImage} />
                ) : selectedPlaceholder ? (
                  <Image
            ...
            <View style={{ marginTop: 2, zIndex: 10 }}>
              {!editIsGlobal && (
                <CityAutocomplete
                  ...
                  onCitySelect={(selectedCity) => {
                    ...
                    if (selectedCity) setEditIsGlobal(false)
                  }}
              ...
              <View style={styles.globalToggleRow}>
                ...
                >
                  {editIsGlobal && <Ionicons name="checkmark" size={12} color="#ffffff" />}
                </TouchableOpacity>
                <Ionicons
                  name="globe-outline"
                  size={14}
