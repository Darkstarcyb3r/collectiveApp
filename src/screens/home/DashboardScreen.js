          ...
          <View style={styles.publicGroupsSection}>
            ...
            >
              <View>
                ...
                {groups.filter(g => g.creatorId === user?.uid).length > 0 && <Text style={styles.publicGroupCounter}>{groups.filter(g => g.creatorId === user?.uid).length}/50 max groups authored</Text>}
            ...
            <TouchableOpacity style={styles.sectionHeaderRow} onPress={isReorderingPrivate ? () => exitReorderMode('private', sortedGroups) : togglePrivateGroups} activeOpacity={0.7}>
              <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                ...
                <View>
                  ...
                  {privateGroupsExpanded && groups.filter(g => g.creatorId === user?.uid).length > 0 && <Text style={styles.groupCounter}>{groups.filter(g => g.creatorId === user?.uid).length}/50 max groups authored</Text>}
